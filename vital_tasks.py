...
class TaskScaleSetup(task.SubjectTask):
    ...
    def __init__(self, subject, init_time, final_time,
        ...
        self.results_scale_path = os.path.join(
                ...
                self.study.name, self.subject.rel_path, 'scale')
        self.output_model_fpath = os.path.join(
                self.study.config['results_path'], 'experiments',
                self.study.name, self.subject.rel_path, '%s.osim' % self.subject.name)
...
class TaskScale(task.SubjectTask):
    ...
    def __init__(self, subject, scale_setup_task,
        ...
        self.residual_actuators_fpath = os.path.join(
                self.study.config['results_path'], 'experiments', self.study.name,
    ...
    def create_residual_actuators(self):
        ...
        def com_in_pelvis():
            '''
            As of version 4.0, Ground is not a Body in the BodySet, it has its own methods model.getGround(), updGround() etc. 
            BodySet refers to all other bodies in the model. Also if using 4.0 you may avoid using the SimbodyEngine class altogether 
            and instead use methods of the "Frame" class. Please let me know if that explains or if you need help writing the script.
            ...
            com_in_ground = m.calcMassCenterPosition(init_state) # not sure if this is actually in ground
            ground = m.getGround()
            pelv = m.getBodySet().get('pelvis')
            com_in_pelvis = ground.expressVectorInAnotherFrame(init_state, com_in_ground, pelv)
