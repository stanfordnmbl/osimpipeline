...
class Trial(object):
    ...
    def __init__(self, condition, num, metadata=None,
        ...
        self.results_exp_path = os.path.join(self.study.config['results_path'],
                'experiments', self.study.name, self.rel_path)
        ...
        #             '//')
        if self.condition.name == 'static':
            self.marker_trajectories_fpath = os.path.join(
                self.expdata_path, 'motion_capture_processed.trc').replace('////', 
                    '//')
        else:
            self.marker_trajectories_fpath = os.path.join(
                self.expdata_path, 'motion_capture.trc').replace('////', 
                    '//')
